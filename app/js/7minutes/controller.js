...
angular.module("workOut")
    .controller("MainController",["$interval","$scope","$location",function($interval,$scope,$location){
       ...
       $scope.str = "ttf";
        ...
        function init(){
            ...
            restExercise = {
                ...
                duration: $scope.workPlan.restBetweenExercise
            };

            $scope.totalTime = $scope.workPlan.exercises.length * 30 +
                restExercise.duration * 11;
            var exercise = $scope.workPlan.exercises.shift();
            //console.log();
        ...
        function getNextExercise(){
            ...
            if($scope.currentExercise === restExercise){
                ...
                if($scope.workPlan.exercises.length > 0){
                    $scope.currentExercise = $scope.workPlan.exercises.shift();
                }else{
                    $scope.currentExercise = null;
                }
            }else {

                $scope.currentExercise = restExercise;
            }
        ...
        function startExercise(exercise){
            ...
            $scope.remaining = exercise.duration;
            ...
            },exercise.duration).then(function(){

                getNextExercise();

                if($scope.currentExercise)
                    startExercise($scope.currentExercise);
                else
        ...
        return function(input){
            ...
            if(isNaN(input)) return "00:00:00";
            var hour = Math.floor(input / 3600);
            var minutes = Math.floor((input % 3600) / 60);
            var second = input % 60;

            return ('0' + hour).substr(-2) + ":"
                    +('0' + minutes).substr(-2) + ":"
                    +('0' + second).substr(-2);
        }
    });